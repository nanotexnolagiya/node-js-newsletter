	...
	var q =
		...
		z = function(e, t) {
			for (var n = 0; n < e.childNodes.length; n++)
				if (f(e.childNodes[n], t)) return e.childNodes[n];
		},
		U = function(e) {
			(e.style.opacity = ''),
		...
		te = function(e) {
			...
			if (
				...
				var p = void 0,
					f = function(e) {
						Ae.isVisible() && p !== e.target.value && Ae.resetValidationError(),
							(p = e.target.value);
					};
				return (
					(i.oninput = f),
					...
					y() ||
						...
						(re.restoreFocusTimeout = setTimeout(function() {
							re.previousActiveElement &&
								re.previousActiveElement.focus &&
								(re.previousActiveElement.focus(),
								(re.previousActiveElement = null));
						}, 100)),
						void 0 !== e && void 0 !== t && window.scrollTo(e, t),
						re.keydownTarget.removeEventListener('keydown', re.keydownHandler, {
							capture: re.keydownListenerCapture,
						}),
	...
	var we = Object.freeze({
			...
			getInput: function(e) {
				...
				switch (e) {
					...
					case 'radio':
						return (
							n.popup.querySelector('.' + D.radio + ' input:checked') ||
							n.popup.querySelector('.' + D.radio + ' input:first-child')
						);
					case 'range':
			...
			_main: function(e) {
				...
				!(function(t) {
						...
						Object.keys(ye).forEach(function(e) {
							t.input === e &&
								(t.inputValidator = t.expectRejections
									? ye[e]
									: Ae.adaptInputValidator(ye[e]));
						}),
						(!t.target ||
							('string' == typeof t.target &&
								!document.querySelector(t.target)) ||
							('string' != typeof t.target && !t.target.appendChild)) &&
							(x('Target parameter is not valid, defaulting to "body"'),
							(t.target = 'body'));
					var e = void 0,
						n = E(),
						o =
					...
					if (
						...
						'string' == typeof t.backdrop
							? (P().style.background = t.backdrop)
							: t.backdrop ||
							  W([document.documentElement, document.body], D['no-backdrop']),
						t.html
							? ne(t.html, c)
							: t.text
								? ((c.textContent = t.text), U(c))
								: F(c),
						t.position in D
							? W(s, D[t.position])
							: (x(
					...
					'function' == typeof t.animation &&
						...
						t.showCloseButton
							? (f.setAttribute('aria-label', t.closeButtonAriaLabel), U(f))
							: F(f),
						(e.className = D.popup),
						t.toast
							? (W([document.documentElement, document.body], D['toast-shown']),
							  W(e, D.toast))
							: W(e, D.modal),
					...
					if (
						(t.imageUrl
							? (C.setAttribute('src', t.imageUrl),
							  ...
							  U(C),
							  t.imageWidth
									? C.setAttribute('width', t.imageWidth)
									: C.removeAttribute('width'),
							  t.imageHeight
									? C.setAttribute('height', t.imageHeight)
						...
						W([d, p], D.styled),
							t.confirmButtonColor &&
								(d.style.backgroundColor = t.confirmButtonColor),
							t.cancelButtonColor &&
								(p.style.backgroundColor = t.cancelButtonColor);
						var k = window
							.getComputedStyle(d)
							.getPropertyValue('background-color');
				...
				var _ = {
					...
					cancelButton: $(),
					closeButton: G(),
					validationError: h(D.validationerror),
					progressSteps: g(),
				};
				...
				return new Promise(function(t, n) {
							...
							(_.container.onmousedown = function() {
								_.popup.onmouseup = function(e) {
									(_.popup.onmouseup = void 0),
										(e.target === _.popup || _.popup.contains(e.target)) &&
											(a = !0);
								};
							}),
					...
					re.keydownHandlerAdded &&
						(re.keydownTarget.removeEventListener(
							'keydown',
							re.keydownHandler,
							{ capture: re.keydownListenerCapture }
						),
						...
						j.toast ||
									...
									if (
										...
										if ('Tab' === e.key) {
											...
											[
												...
												? document.activeElement === _.confirmButton &&
												  K(_.cancelButton)
													? _.cancelButton.focus()
													: document.activeElement === _.cancelButton &&
													  K(_.confirmButton) &&
							...
							re.keydownTarget.addEventListener('keydown', re.keydownHandler, {
								capture: re.keydownListenerCapture,
							}),
						...
						T.hideLoading(),
						T.resetValidationError(),
						(j.input || j.footer || j.showCloseButton) &&
							W(document.body, D['toast-column']);
					for (
						var u = [
								'input',
								'file',
					...
					switch (j.input) {
						...
						case 'url':
							...
							break;
						case 'file':
							((p = z(_.content, D.file)).placeholder = j.inputPlaceholder),
								(p.type = j.input),
								U(p);
							break;
						case 'range':
						...
						case 'select':
							...
							b = function(e) {
								e.forEach(function(e) {
									...
									(r.value = n),
										(r.innerHTML = o),
										j.inputValue.toString() === n.toString() &&
											(r.selected = !0),
										x.appendChild(r);
								}),
									U(x),
						...
						case 'checkbox':
							...
							E.length && S.removeChild(E[0]),
								((E = document.createElement('span')).className = D.label),
								(E.innerHTML = j.inputPlaceholder),
								S.appendChild(E),
								U(S);
							break;
						case 'textarea':
					...
					if ('select' === j.input || 'radio' === j.input) {
						...
						-1 !==
							['text', 'email', 'number', 'tel', 'textarea'].indexOf(j.input) &&
							R(j.inputValue) &&
							(V.showLoading(),
							F(p),
							j.inputValue
								.then(function(e) {
									(p.value =
										'number' === j.input ? parseFloat(e) || 0 : e + ''),
										U(p),
										p.focus(),
										T.hideLoading();
								})
	...
	var xe,
		...
		Ae = j(
			...
			'undefined' != typeof window &&
				...
				Be.setDefaults(window._swalDefaults),
			Be)
		);
